...
def get_top_two(test_df: pd.core.frame.DataFrame) -> pd.core.frame.DataFrame:
    ...
    return copied_df.sort_values('area', ascending=False).head(2)
...
def population_density(test_df: pd.core.frame.DataFrame) -> pd.core.frame.DataFrame:
    copied_df = test_df.copy()
    copied_df['density'] = copied_df['population'] / copied_df['area']
    return copied_df
...
# %%
def plot_population(test_df: pd.core.frame.DataFrame) -> mpl.figure.Figure:
    fig, ax = plt.subplots()
...
# %%
def plot_area(test_df: pd.core.frame.DataFrame) -> mpl.figure.Figure:
    fig, ax = plt.subplots()
